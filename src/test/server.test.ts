...
import { analyzeMessage, scanUrl, checkPhone } from "../shared";
...
function createApp() {
  ...
  app.post("/check-phone", (req, res) => {
    ...
    if (typeof number !== "string") return res.status(400).json({ error: "number must be a string" });
    return res.json(checkPhone(number));
  });
  return app;
}
